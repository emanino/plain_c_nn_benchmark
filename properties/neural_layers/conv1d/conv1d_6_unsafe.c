...
int main() /* check_elimination_kernel_size_7 */
{
	float x[INPUT_SIZE], y[INPUT_SIZE - KERNEL_SIZE + 1];
	
	float kernel[KERNEL_SIZE] = {1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f};
	...
	__VERIFIER_assert(y[0] + y[1] == x[0] + x[7]); /* Expected result: verification failure */
